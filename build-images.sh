...
# Reuse existing nodebuilder-coder container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-coder; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-coder -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:14.2 ghcr.io/coder/coder:${CODER_TAG} docker.io/codercom/code-server:ubuntu" \
    "${container}"
# Commit the image
